...
public class SlotServiceTest {
	...
	@Test(expected = SlotNotFoundException.class)
	public void testBookSlotForSlotNotFoundException() throws SlotNotFoundException, UserNotFoundException {
		bookSlotRequestDto.setSlotId(1);
		Mockito.when(slotRepository.findById(3)).thenReturn(Optional.of(slot));
		slotServiceImpl.bookSlot(bookSlotRequestDto);
	}

	@Test(expected = UserNotFoundException.class)
	public void testBookSlotForUserNotFoundException() throws SlotNotFoundException, UserNotFoundException {
		bookSlotRequestDto.setSlotId(1);
		bookSlotRequestDto.setUserId(1);
		Mockito.when(slotRepository.findById(1)).thenReturn(Optional.of(slot));
		Mockito.when(userRepository.findById(3)).thenReturn(Optional.of(user));
		slotServiceImpl.bookSlot(bookSlotRequestDto);
	}

	@Test(expected = SlotNotFoundException.class)
	public void testBookSlotForSlotAlreadyExistException() throws SlotNotFoundException, UserNotFoundException {
		bookSlotRequestDto.setSlotId(1);
		bookSlotRequestDto.setUserId(1);
		slot.setSlotStatus(SlotStatus.BOOKED);
		Mockito.when(slotRepository.findById(1)).thenReturn(Optional.of(slot));
	...
	@Test 
	public void testGetListForNegative() { 
		slot.setDate(LocalDate.of(2020, 02, 19));
		slot.setFromTime(LocalTime.of(2, 25, 24));
		slot.setToTime(LocalTime.of(2, 25, 35));
		slots = new ArrayList<>();
	  Mockito.when(slotRepository.SlotByDateFromTimeToTimeStatus(LocalDate.of(2020, 02, 19), LocalTime.of(2, 25, 24),
			  LocalTime.of(2, 25, 35))).thenReturn(slots);
	  Boolean actual =slotServiceImpl.findList(LocalDate.of(2020, 02, 19), LocalTime.of(2, 25, 24), LocalTime.of(2, 25, 35));
	...
	@Test 
	public void testGetListForPositive() { 
		slot.setSlotId(1);
		slot.setDate(LocalDate.of(2020, 02, 19));
		slot.setFromTime(LocalTime.of(2, 25, 24));
		slot.setToTime(LocalTime.of(2, 25, 35));
	...
	@Test (expected = SlotNotFoundException.class)
	public void testBookSlotForSlotAlreadyExist() throws SlotNotFoundException, UserNotFoundException { 
		slot.setSlotId(1);
		slot.setDate(LocalDate.of(2020, 02, 19));
		slot.setFromTime(LocalTime.of(2, 25, 24));
		slot.setToTime(LocalTime.of(2, 25, 35));
	  ...
	  Mockito.when(slotRepository.SlotByDateFromTimeToTimeStatus(LocalDate.of(2020, 02, 19), LocalTime.of(2, 25, 24),
			  LocalTime.of(2, 25, 35))).thenReturn(slots);
	  slotServiceImpl.findList(LocalDate.of(2020, 02, 19), LocalTime.of(2, 25, 24), LocalTime.of(2, 25, 35));
	  slotServiceImpl.bookSlot(bookSlotRequestDto);
	  }
